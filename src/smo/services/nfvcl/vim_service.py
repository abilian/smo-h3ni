...
def sync_vims(nfvcl_base_url):
    ...
    for vim_data in topology_data.get("vims", []):
        ...
        existing_vim = (
            db.session.query(VIM).filter_by(vim_name=vim_data.get("name", "")).first()
        )
        if existing_vim is None:
            vim = VIM(
                ...
                use_floating_ip=vim_data.get("config", {}).get(
    ...
    if added_vims:
        ...
        if existing_vims_data:
            return {
                "message": "No new VIMs obtained by the NFVCL. Existing VIMs:",
                "vims": existing_vims_data,
            }
        else:
            return {
                "message": "No new VIMs obtained by the NFVCL and no existing VIMs in the database."
            }
...
def remove_vim(smo_id):
    ...
    db.session.commit()
    return {
        "message": f"VIM with SMO ID {smo_id} deleted successfully",
