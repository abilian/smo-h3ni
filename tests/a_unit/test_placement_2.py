...
def test_convert_placement_valid_input():
    ...
    expected_map = {
        "app1-svc1": "kube-main",
        "app1-svc2": "gpu-cluster",
        "app2-svc1": "kube-main",
    }

    converted_map = placement.convert_placement(
        placement_mat, services_info_list, cluster_name_list
    )
...
def test_convert_placement_unplaced_service_raises_value_error():
    ...
    cluster_names_unplaced = [
        "clusterA",
        "clusterB",
        "clusterC",
    ]  # Ensure num_clusters matches matrix width

    with pytest.raises(PlacementError):
        placement.convert_placement(
            placement_mat_unplaced, services_info_unplaced, cluster_names_unplaced
        )
...
def test_decide_placement_with_suboptimal_current_placement():
    ...
    optimal_placement_reopt = placement.decide_placement(
        ...
        dp_accel_reqs,
        dp_replicas,
        dp_current_placement_suboptimal,
    )
...
def test_calculate_naive_placement_feasible():
    ...
    naive_placement = placement.calculate_naive_placement(
        np_cluster_caps, np_cluster_accel, np_cpu_limits, np_accel_reqs, np_replicas
    )
...
def test_calculate_naive_placement_insufficient_capacity_raises_value_error():
    ...
    with pytest.raises(PlacementError):
        placement.calculate_naive_placement(
            np_cluster_caps_tight,
            np_cluster_accel_tight,
            np_cpu_limits_tight,
            np_accel_reqs_tight,
            np_replicas_tight,
        )
